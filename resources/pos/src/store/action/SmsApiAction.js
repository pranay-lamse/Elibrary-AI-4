...
export const fetchSmsApiSetting = () => async (dispatch) => {
    await apiConfig
        .get(apiBaseURL.SMS_SETTING)
        .then((response) => {
            if (response) {
                dispatch({
                    type: smsApiActionType.FETCH_SMS_SETTINGS,
...
export const updateSmsApiSetting = (smsData) => async (dispatch) => {
    await apiConfig
        .post(apiBaseURL.SMS_SETTING, smsData)
        .then((response) => {
            if (response) {
                dispatch({
                    type: smsApiActionType.EDIT_SMS_SETTING,
                    payload: response.data.data,
                });
                dispatch(
                    addToast({
                        text: getFormattedMessage(
                            "sms.api.update.success.message"
                        ),
