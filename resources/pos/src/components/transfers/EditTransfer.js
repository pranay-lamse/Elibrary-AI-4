...
const EditTransfer = (props) => {
    ...
    const selectedStatus =
        tansfers.attributes &&
        tansfers.attributes.status &&
        TransferStatusType.filter(
            (item) => item.value === tansfers.attributes.status
        );

    const itemsValue = tansfers &&
        tansfers.attributes && {
            ...
            transfer_items: tansfers.attributes.transfer_items.map((item) => ({
                ...
                stock:
                    ...
                    item.product.stocks.filter(
                        (item) =>
                            item.warehouse_id ===
                            tansfers.attributes.from_warehouse_id
                    ),
    ...
    return (
        <MasterLayout>
            <HeaderTitle
                title={getFormattedMessage("transfer.edit.title")}
                to="/admin/pos/transfers"
            />
            {isLoading ? (
                <div className="text-center custom-loading mx-auto fs-1 fw-bold">
                    Loading...
                </div>
            ) : (
                <TransferForm
                    singleTransfer={itemsValue}
                    id={id}
