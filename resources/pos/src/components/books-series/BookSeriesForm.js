...
import Select from "../../shared/components/SelectSeries";
...
const BookSeriesForm = (props) => {
    ...
    const [image, isDefaultImage, file, onFileChange, onRemovePhoto] =
        imagePicker(
            change,
            initialValues?.cover
                ? "public/uploads/series/" + initialValues?.cover
    ...
    const prepareFormData = (formValues) => {
        ...
        formData.append(
            "series_items",
            JSON.stringify(formValues.series_items)
        );
    ...
    const onSave = (formValues) => {
        if (initialValues?.cover) {
            formValues.cover = initialValues.cover;
        }

        if (image.includes("data")) {
            formValues.cover = file;
        }
    ...
    return (
        <Row className="animated fadeIn m-none m-sm-3">
            <Col xs={12} className="book-form-card__book-cover">
                <h5>Series Cover</h5>
                <hr />
                <div className="mt-5">
                    <Field
            ...
            <Col xs={12}>
                <Field
                    name="title"
                    label="books-series.input.title.label"
                    required
                    inputRef={inputRef}
                    groupText="television"
                    component={InputGroup}
                />
            </Col>
            <Col xs={12} className="mt-3">
                ...
                <FieldArray
                    name="series_items"
                    component={renderBookSeriesItems}
...
const renderBookSeriesItems = ({
    ...
    if (fields.length === 0) {
        return (
            <div>
                <EmptyComponent isShort={true} title={"No Records Found."} />
                <button
                    type="button"
                ...
                >
                    {getFormattedMessage(
                        "books-series.items.input.add-item-btn.label"
                    )}
                </button>
            </div>
        );
    ...
    return (
        <div className="overflow-auto">
            <Table responsive size="md">
                <thead>
                    <tr>
                        <th className="header-responsive">
                            {getFormattedMessage(
                                "books-series.items.input.sequence.label"
                            )}
                        </th>
                        <th className="book-form__item-header header-responsive">
                            {getFormattedMessage(
                                "books-series.items.select.book-name.label"
                            )}
                        </th>
                    ...
                    <Droppable droppableId="droppable">
                        {(provided, snapshot) => (
                            ...
                            >
                                            ...
                                            {(provided, snapshot) => (
                                                ...
                                                >
                                                    ...
                                                    <td className="text-center">
                                                        ...
                                                        >
                                                            <i className="fas fa-trash-alt icon font-md text-white" />
                                                        </Button>
                                                    </td>
                                ...
                                {provided.placeholder}
                                {index !== null ? (
                                    <DeleteBookSeriesItem {...cardModalProps} />
                                ) : null}
                            </tbody>
                        )}
                    </Droppable>
...
const bookSeriesForm = reduxForm({
    form: "bookSeriesForm",
    validate: bookSeriesValidate,
})(BookSeriesForm);
...
export default connect(mapStateToProps, { fetchBooks, toggleModal })(
    bookSeriesForm
);
