        ...
        <Row>
            <Col xs={12}>
                ...
                <span className="ml-2 text-primary">
                    {file
                        ? file.name
                        : getFormattedMessage(
                              "books.items.no-file-chosen-text"
                          )}
                </span>
            </Col>
            <Col xs={12} className="mt-2">
                <span className="sample-file-text">
                    {getFormattedMessage(
                        "books.items.download-sample-file.text"
                    )}
                </span>
                <span className="ml-1">
                    <a
                        target="_blank"
                        href={`${environment.URL}/public/uploads/sample.xlsx`}
                    >
...
ImportMemberForm.propTypes = {
    onSaveImportData: PropTypes.func,
};

export default reduxForm({ form: "importMemberForms" })(ImportMemberForm);
