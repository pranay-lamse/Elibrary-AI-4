 ...
 */
class TransferRepository3 extends BaseRepository
{
    /**
     * @var array
     */
    ...
    public function storeTransfer($input)
    {
        try {
            ...
            $transfer = Transfer3::create($TransferInputArray);
    ...
    {
        $validator = Validator::make($transferItem, TransferItem3::$rules);
        if ($validator->fails()) {
        ...
        if ($transferItem['discount_type'] == Transfer3::PERCENTAGE) {
        ...
        } elseif ($transferItem['discount_type'] == Transfer3::FIXED) {
        ...
        if ($transferItem['tax_value'] <= 100 && $transferItem['tax_value'] >= 0) {
            if ($transferItem['tax_type'] == Transfer3::EXCLUSIVE) {
            ...
            } elseif ($transferItem['tax_type'] == Transfer3::INCLUSIVE) {
    ...
    public function updateTransfer($input, $id)
    {
        try {
            ...
            $transfer = Transfer3::findOrFail($id);

            $transferItemOldIds = TransferItem3::whereTransferId($id)->pluck('id')->toArray();
            $transferItemNewIds = [];
            ...
            foreach ($input['transfer_items'] as $key => $transferItem) {
                ...
                if (is_null($transferItem['transfer_item_id'])) {
                    ...
                    $transferItem = new TransferItem3($item);
            ...
            if (!empty(array_values($removeItemIds))) {
                foreach ($removeItemIds as $removeItemId) {
                    $oldTransferItem = TransferItem3::whereId($removeItemId)->first();
                    $oldTransfer = Transfer3::whereId($oldTransferItem->transfer_id)->first();
    ...
    public function updateItem($transferItem, $fromWarehouseId, $toWarehouseId): bool
    {
        try {
            ...
            $item = TransferItem3::whereId($transferItem['transfer_item_id'])->first();
            $transfer = Transfer3::whereId($item->transfer_id)->first();
