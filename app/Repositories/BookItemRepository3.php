 ...
 */
class BookItemRepository3 extends BaseRepository3 implements BookItemRepositoryInterface3
{
    /**
     * @var array
     */
    ...
    public function searchBooks($search = [], $skip = null, $limit = null)
    {
        $query = $this->allQuery($search, $skip, $limit)->with([
            ...
            'language',
            'book.genres',
            'ebooksubscriptions' => function ($query) {
    ...
    {
        $available = collect($records->where('status', BookItem3::STATUS_AVAILABLE));
        $notAvailable = collect($records->where('status', BookItem3::STATUS_NOT_AVAILABLE))
