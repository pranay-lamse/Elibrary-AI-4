 ...
 */
class Product2 extends BaseModel implements HasMedia, JsonResourceful
{
    ...
    {
        /** @var Media $media */
        $medias = $this->getMedia(Product2::PATH);
        $images = [];
        if (!empty($medias)) {
    ...
    {
        /** @var Media $media */
        $media = $this->getMedia(Product2::PRODUCT_BARCODE_PATH)->first();
        if (!empty($media)) {
    ...
    public function getIdFilterFields(): array
    {
        return [
            'id'                  => self::class,
            'product_category_id' => ProductCategory2::class,
            'brand_id'            => Brand2::class,
        ];
    }
    ...
    {
        $purchaseUnit = Unit2::whereId($this->purchase_unit)->first();
        if ($purchaseUnit) {
    ...
    {
        $saleUnit = Unit2::whereId($this->sale_unit)->first();
        if ($saleUnit) {
    ...
    {
        return $this->belongsTo(ProductCategory2::class, 'product_category_id', 'id');
    }
    ...
    {
        return $this->belongsTo(Brand2::class, 'brand_id', 'id');
    }
    ...
    {
        return $this->hasOne(ManageStock2::class, 'product_id', 'id');
    }
    ...
    {
        return $this->hasMany(Purchase2::class, 'purchase_id', 'id');
    }
    ...
    {
        return Managestock2::where('product_id', $id)->Join(
            'warehouses',
    ...
    public function bookItemCount($id)
    {
        ...
        return BookItem2::where('product_id',  $id)->count();
    }
    ...
    {
        $totalQuantity = Managestock2::where('product_id', $id)->sum('quantity');
    ...
    {
        return $this->hasMany(ManageStock2::class, 'product_id', 'id');
    }
